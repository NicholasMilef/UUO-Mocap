...
from video_mocap.datasets.smplh_dataset import SMPLHDataset
from video_mocap.utils.smpl_utils import smpl_joint_symmetry


class DatasetMocap(Dataset):
    def __init__(
        self,
        split="train",
        num_markers=43,
        ...
        if self.split == "train":
            self.dataset_names = ["ACCAD", "BioMotionLab_NTroje", "BMLmovi", "EKUT",  "Eyes_Japan_Dataset", "KIT", "MPI_Limits"]
        ...
        self.marker_dataset = SMPLHDataset(batch_size * self.num_markers, torch.device("cpu"), parts_set=parts_set, seed=seed)
    ...
    def compute_markers(
        ...
        vertex_part_labels: torch.Tensor=None,  # [V, J]
        marker_offset: float=0.0095,  # consistent with SOMA (9.5mm offset)
    ) -> Dict:
        # compute rotation orientation
        batch_size, num_frames, num_vertices, _ = vertices.shape
        device = vertices.device

        markers = self.marker_dataset[0]

        markers["pos"] = markers["pos"].to(device)
        markers["barycentric_coords"] = markers["barycentric_coords"].to(device)
        markers["triangles"] = markers["triangles"].to(device)

        marker_pos = torch.reshape(markers["pos"], (batch_size, self.num_markers, 3))  # [N, M, 3]
        marker_triangles = torch.reshape(markers["triangles"], (batch_size, self.num_markers, 3))  # [N, M, 3]
        marker_barycentric_coords = torch.reshape(markers["barycentric_coords"], (batch_size, 1, self.num_markers, num_vertices, 1))  # [N, 1, M, V, 1]
        marker_barycentric_coords_pos = torch.repeat_interleave(marker_barycentric_coords, dim=-1, repeats=3)  # [N, 1, M, V, 3]
        if vertex_part_labels is not None:
            num_parts = vertex_part_labels.shape[-1]
            marker_barycentric_coords_weights = torch.repeat_interleave(marker_barycentric_coords, dim=-1, repeats=num_parts)  # [N, 1, M, V, P]
            marker_body_part = torch.reshape(vertex_part_labels, (batch_size, 1, 1, 6890, num_parts))

        vertices_rs = torch.unsqueeze(vertices, dim=2)  # [N, F, 1, V, 3]

        output = {}

        output["marker_pos"] = torch.sum(marker_barycentric_coords_pos * vertices_rs, dim=3)  # [N, F, M, 3]
        output["marker_pos_template"] = marker_pos  # [N, M, 3]

        ab = vertices_rs[:, :, :, marker_triangles[0, :, 1], :] - vertices_rs[:, :, :, marker_triangles[0, :, 0], :]
        ac = vertices_rs[:, :, :, marker_triangles[0, :, 2], :] - vertices_rs[:, :, :, marker_triangles[0, :, 0], :]
        cross = torch.linalg.cross(ab, ac, dim=-1)  # [N, F, 1, M, 3]
        normals = (cross / torch.norm(cross, dim=-1, keepdim=True))[:, :, 0, :, :]
        output["marker_pos"] = output["marker_pos"] + (normals * marker_offset)

        if vertex_part_labels is not None:
            output["marker_part_labels"] = torch.sum(marker_barycentric_coords_weights * marker_body_part, dim=3)[:, 0]  # [N, M, P]

        return output


    def compute_markers_fast(
        self,
        vertices: torch.Tensor,
        vertex_part_labels: torch.Tensor=None,
    ) -> torch.Tensor:
        raise ValueError("Do not use yet")
        ...
        marker_pos = torch.reshape(markers["pos"], (batch_size, self.num_markers, 3))  # [N, M, 3]
        marker_barycentric_coords = torch.reshape(markers["barycentric_coords"], (batch_size, 1, self.num_markers, num_vertices))  # [N, M, V]

        marker_indices = torch.argmax(marker_barycentric_coords, dim=-1, keepdim=True)  # [N, 1, M, 1]
        marker_indices = torch.repeat_interleave(marker_indices, repeats=num_frames, dim=1)  # [N, F, M, 1]
        marker_indices = torch.repeat_interleave(marker_indices, repeats=3, dim=-1)  # [N, F, M, 3]

        output = {}

        output["marker_pos"] = torch.gather(vertices, 2, marker_indices)  # [N, F, M, 3]
        output["marker_pos_template"] = marker_pos  # [N, M, 3]

        if vertex_part_labels is not None:
            num_parts = vertex_part_labels.shape[-1]
            marker_indices_part = torch.repeat_interleave(marker_indices[:, 0, :, [0]], repeats=num_parts, dim=-1)  # [N, M, P]
            marker_body_part = torch.reshape(vertex_part_labels, (batch_size, num_vertices, num_parts))  # [N, V, P]
            output["marker_part_labels"] = torch.gather(marker_body_part, 1, marker_indices_part)  # [N, M, P]
